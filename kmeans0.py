...
from aver import calculate_average_by_year, WORLD_DATA


warnings.filterwarnings("ignore")  # 忽略无关警告


def kmeans_clustering_world_data():
    avg = calculate_average_by_year(WORLD_DATA)
    ...
    print(
        f"🔋 碳排放特征：{emission_feature_dim}维（{sorted_years[0]}-{sorted_years[-1]}年每年均值）"
    )
    ...
    double_dim_features = (
        []
    )  # 双维度特征矩阵（每行：[纬度, 经度, 2019年排放, 2020年排放, ...]）
    ...
    for cluster_id in range(n_clusters):
        ...
        lat_range = (
            f"{cluster_data['纬度'].min():.2f} - {cluster_data['纬度'].max():.2f}"
        )
        lon_range = (
            f"{cluster_data['经度'].min():.2f} - {cluster_data['经度'].max():.2f}"
        )
    ...
    print(
        f"📋 结果包含：城市名称、经纬度、聚类标签、2019年排放均值、各年份原始排放数据"
    )
    print("=" * 70)

    # 可视化全球城市双维度KMeans聚类结果 ================================

    # 2. 加载聚类结果数据
    try:
        file_path = "全球城市双维度聚类结果（经纬度+碳排放）.csv"
        df = pd.read_csv(file_path)
        print(f"✅ 成功加载聚类结果文件: {file_path}")
        print(f"共加载 {len(df)} 个城市的数据。")
    except FileNotFoundError:
        print(f"❌ 错误：找不到文件 '{file_path}'。")
        print("请先运行之前的聚类分析代码以生成该文件。")
        exit()

    # 3. 创建基础世界地图
    # 设置一个合适的初始中心点和缩放级别，以展示全球概览
    ...
    for index, row in df.iterrows():
        ...
        lon = row["经度"]
        city_name = row["城市名称"]
        cluster_label = row["聚类标签"]
        emission_2019 = row["2019年碳排放均值"]

        # 创建弹出窗口的HTML内容，展示详细信息
        popup_html = f"""
        <h4>{city_name}</h4>
        <hr>
        <b>聚类标签:</b> {cluster_label}<br>
        <b>2019年碳排放均值:</b> {emission_2019:.2f}
        """

        # 创建一个 IFrame 以便更好地控制弹出窗口样式
        iframe = folium.IFrame(popup_html, width=200, height=100)
        popup = folium.Popup(iframe, max_width=200)

        # 添加圆形标记 (CircleMarker)
        # CircleMarker的半径以像素为单位，不会随地图缩放而改变，适合展示点数据
        folium.CircleMarker(
            location=[lat, lon],
            radius=5,  # 标记的大小
            color=color_map[cluster_label],  # 标记的边框颜色
            fill=True,
            fill_color=color_map[cluster_label],  # 标记的填充颜色
            fill_opacity=0.7,  # 填充的透明度
            popup=popup,  # 点击时显示的弹出窗口
            tooltip=f"{city_name} (Cluster {cluster_label})",  # 鼠标悬停时显示的提示
        ).add_to(world_map)

    print("✅ 所有城市标记添加完毕。")

    # 6. 添加图例
    # Folium没有内置的简单图例，我们可以手动创建一个HTML图例
    legend_html = """
    ...
    for i in range(n_clusters):
        cluster_name = f"聚类 {i}"
        color = color_map[i]
        legend_html += f'<i style="background:{color}; width:20px; height:20px; border-radius:50%; display:inline-block; margin-right:5px; vertical-align:middle;"></i>{cluster_name}<br>'
    ...
    # 7. 保存地图为HTML文件
    output_file = "全球城市双维度聚类可视化.html"
    world_map.save(output_file)

    print("\n" + "=" * 70)
    print(f"🎉 可视化地图已成功生成！")
    print(f"请在浏览器中打开文件: {output_file}")
    print("您可以进行缩放、平移，并点击城市标记查看详细信息。")
    print("=" * 70)
