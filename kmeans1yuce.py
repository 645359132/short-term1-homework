...
def prepare_city_features(data):
    ...
    for city in data['city_name'].unique():
        ...
        city_features.append([
            total_emission, mean_emission, std_emission,
            lat, lon, years_count, slope
        ])

    feature_names = [
        'total_emission', 'mean_emission', 'std_emission',
        'lat', 'lon', 'years_count', 'trend_slope'
    ]
...
def analyze_and_visualize(data):
    ...
    for k in k_range:
        ...
        inertia = sum([np.sum((X_normalized[labels == i] - centroids[i])**2)
    ...
    for i in range(optimal_k):
        ...
        cluster_stats[i] = {
            ...
            'trend_desc': '上升趋势' if cluster_data[:, 6].mean() > 0.1 else '下降趋势' if cluster_data[:, 6].mean() < -0.1 else '相对稳定'
        }

    # 创建增强版可视化
    fig = plt.figure(figsize=(20, 16))
    fig.suptitle(f'基于7维特征的城市碳排放K-means聚类分析 (K={optimal_k})\n' +
    ...
    for i in range(optimal_k):
        mask = labels == i
        scatter = ax1.scatter(X[mask, 4], X[mask, 3], c=[colors[i]],
                   label=f'聚类{i} ({cluster_stats[i]["count"]}城市)',
                   s=80, alpha=0.7, marker=markers[i], edgecolors='black', linewidth=0.5)

    centroids_original = np.array([X[labels == i].mean(axis=0) for i in range(optimal_k)])
    ax1.scatter(centroids_original[:, 4], centroids_original[:, 3],
               c='red', marker='X', s=300, linewidths=2, label='聚类中心', edgecolors='black')

    ax1.set_xlabel('🌍 特征5: 经度 (Longitude)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('🌍 特征4: 纬度 (Latitude)', fontsize=12, fontweight='bold')
    ax1.set_title('地理位置特征聚类展示\n基于7维特征聚类，展示地理位置维度',
    ...
    for i in range(optimal_k):
        mask = labels == i
        scatter = ax2.scatter(X[mask, 0], X[mask, 1], c=[colors[i]],
    ...
    bars = ax3.bar(unique, counts, color=colors[:len(unique)], alpha=0.7, edgecolor='black', linewidth=2)
    ...
    bars = ax4.bar(range(optimal_k), trend_means, color=colors, alpha=0.7, edgecolor='black', linewidth=2)
    ...
    ax4.set_title('排放趋势特征聚类展示\n基于7维特征聚类，展示时间趋势维度',
                  fontsize=12, fontweight='bold', color='darkred')
    ax4.grid(True, alpha=0.3)

    textstr = '展示特征:\n• 排放趋势斜率\n  (正值=上升趋势)\n  (负值=下降趋势)\n\n聚类依据:\n基于全部7个特征'
    props = dict(boxstyle='round', facecolor='lightpink', alpha=0.7)
    ax4.text(0.02, 0.02, textstr, transform=ax4.transAxes, fontsize=8,
             verticalalignment='bottom', bbox=props)

    # 5. 排放波动性特征展示 (特征3: 标准差)
    ax5 = plt.subplot(2, 3, 5)
    std_means = [cluster_stats[i]['std_emission_mean'] for i in range(optimal_k)]
    bars = ax5.bar(range(optimal_k), std_means, color=colors, alpha=0.7, edgecolor='black', linewidth=2)
    ...
    # 雷达图显示各聚类在7个特征上的表现
    feature_labels_chinese = ['总排放量', '平均排放量', '排放波动性', '纬度', '经度', '数据年份', '排放趋势']
    ...
    for i in range(optimal_k):
        ...
        if len(cluster_data) > 0:
            ...
            ax.plot(angles, radar_values, 'o-', linewidth=3,
                   label=f'聚类{i} ({cluster_stats[i]["count"]}城市)',
                   color=colors[i], alpha=0.8, markersize=8)
    ...
    ax.text(1.3, 0.5, feature_explanation, transform=ax.transAxes, fontsize=10,
    ...
    for i in range(optimal_k):
        ...
        for j, (name, desc) in enumerate(feature_descriptions.items()):
            ...
            level = "高于平均" if normalized_value > 0.5 else "低于平均" if normalized_value < -0.5 else "接近平均"
            print(f"  • {desc}: {value:.2f} ({level})")

        # 聚类特征总结
        print(f"\n🎯 聚类特征总结:")
        emission_level = "高排放" if cluster_stats[i]['mean_emission_mean'] > 30 else "中排放" if cluster_stats[i]['mean_emission_mean'] > 10 else "低排放"
        stability = "波动大" if cluster_stats[i]['std_emission_mean'] > 20 else "相对稳定"
        print(f"  • 排放水平: {emission_level}")
        print(f"  • 排放稳定性: {stability}")
        print(f"  • 发展趋势: {cluster_stats[i]['trend_desc']}")
        print(f"  • 地理分布: 纬度{cluster_stats[i]['lat_mean']:.1f}°, 经度{cluster_stats[i]['lon_mean']:.1f}°")
...
def linear_trend_with_confidence(time_series, future_years, confidence=0.95):
    ...
    # 标准误差
    se = np.sqrt(mse * (1 + 1 / len(x) + (future_x - x_mean) ** 2 / np.sum((x - x_mean) ** 2)))
...
def ensemble_prediction(predictions, future_years):
    ...
    if not valid_predictions:
        return {'predictions': np.zeros(future_years), 'method': 'Ensemble (no valid predictions)'}
...
def perform_multiple_predictions(time_series, future_years):
    ...
    # 4. 指数平滑预测
    predictions['exponential'] = exponential_smoothing_prediction(time_series, future_years)
...
def enhanced_prediction(data, results_df, future_years=5):
    ...
    for cluster_id in sorted(results_df['cluster'].unique()):
        cluster_cities = results_df[results_df['cluster'] == cluster_id]['city_name'].tolist()
        ...
        # 按年聚合
        time_series = cluster_data.groupby('year').agg({
            'value': 'sum'
        }).reset_index()
        ...
        # 可视化结果
        visualize_enhanced_predictions(time_series, predictions, cluster_id, future_years)
...
def visualize_enhanced_predictions(time_series, predictions, cluster_id, future_years):
    ...
    if 'linear' in predictions:
        ...
        ax1.plot(future_dates, linear_pred['predictions'], 'r--',
                 label=f"线性预测 (R²={linear_pred['r_squared']:.3f})", linewidth=2)
        ax1.fill_between(future_dates, linear_pred['lower_bound'], linear_pred['upper_bound'],
    ...
    if 'polynomial' in predictions:
        ...
        ax2.plot(future_dates, poly_pred['predictions'], 'g--',
                 label=f"{poly_pred['method']}", linewidth=2)

    if 'moving_average' in predictions:
        ma_pred = predictions['moving_average']
        ax2.plot(future_dates, ma_pred['predictions'], 'm--',
                 label=f"{ma_pred['method']}", linewidth=2)
    ...
    if 'exponential' in predictions:
        ...
        ax3.plot(future_dates, exp_pred['predictions'], 'orange', linestyle='--',
                 label=f"{exp_pred['method']}", linewidth=2)
    ...
    for method, pred_data in predictions.items():
        if 'predictions' in pred_data and i < len(colors):
            ax4.plot(future_dates, pred_data['predictions'],
                     color=colors[i], linestyle='--', alpha=0.7,
                     label=pred_data['method'], linewidth=1.5)
            i += 1

    # 突出显示集成预测
    if 'ensemble' in predictions:
        ensemble_pred = predictions['ensemble']
        ax4.plot(future_dates, ensemble_pred['predictions'], 'black', linestyle='-',
                 label='集成预测', linewidth=3)
    ...
    for method, pred_data in predictions.items():
        if 'predictions' in pred_data:
            ...
            if method == 'linear' and 'slope' in pred_data:
                trend = "上升" if pred_data['slope'] > 0 else "下降" if pred_data['slope'] < 0 else "稳定"
                print(f"  趋势方向: {trend}")
