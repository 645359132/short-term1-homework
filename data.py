...
class DataLoader(object):
    ...
    def _data_clean(self):
        ...
        if len(existing_critical_cols) != len(critical_cols):
            print(f"警告：某些关键列不存在！期望：{critical_cols}, 实际存在：{existing_critical_cols}")
        ...
        print(f"因 {existing_critical_cols} 列存在缺失值而删除了 {original_rows - len(df)} 行。")
    ...
    def _add_time_features(self):
        ...
        df_featured['is_weekend'] = (df_featured['day_of_week'] >= 5).astype(int)  # 0 for weekday, 1 for weekend
